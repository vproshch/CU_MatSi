...
save_file_name = 'test_paul'; %in here, put what you want the output files to be called
...
load('point_1_mesh_8x8x8.mat');         %load('point_1_mesh.mat')
...
potential_sites1 = replicateSite(original,a,xhi,xlo,yhi,ylo,zhi,zlo);

spot_2 = original + [a/2 ;a/2; 0 ];
potential_sites2 = replicateSite(spot_2,a,xhi,xlo,yhi,ylo,zhi,zlo);

potential_sites = [potential_sites1 potential_sites2   ];

spot_3 = original + [ 0; a/2; a/2 ];
potential_sites3 = replicateSite(spot_3,a,xhi,xlo,yhi,ylo,zhi,zlo);

potential_sites = [potential_sites1 potential_sites2 potential_sites3   ];

spot_4 = original + [ a/2; 0; a/2 ];
potential_sites4 = replicateSite(spot_4,a,xhi,xlo,yhi,ylo,zhi,zlo);

potential_sites = [potential_sites1 potential_sites2 potential_sites3 potential_sites4  ];
% 
...
intSpot = inters(:,p);
    ...
    for i = 1:(length(Pos)) %for every atom excluding itself
      ...
      if ~isequal(intSpot, Pos(:,i))
        intDists(p,i) = getDistance(intSpot,Pos(:,i));
        
        if intDists(p,i) < 0.3
            fprintf('inters %d is very close to atom %d\n',p,i); 
        end
    ...
    if p < 20 %only plot for this range 
        figure(1+1+p)
        scatter(1:20,intDists(p,1:20));
        xlabel('atom #');
        ylabel('distance (A)');
...
function [potential_sites] = replicateSite(original,a,xhi,xlo,yhi,ylo,zhi,zlo)

%replicating to find possible spots

 potential_sites = original;
